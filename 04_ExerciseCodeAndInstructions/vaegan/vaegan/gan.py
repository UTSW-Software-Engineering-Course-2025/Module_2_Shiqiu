...
class GAN(tf.keras.Model):
    ...
    def __init__(self, 
        ...
        self.generator = Generator(self.image_shape)
        ...
        self.loss_bce = tf.keras.losses.BinaryCrossentropy(name='binary_crossentropy')
        ...
        self.optimizer_gen = tf.keras.optimizers.legacy.Adam(learning_rate = 0.001)
        self.optimizer_disc = tf.keras.optimizers.legacy.Adam(learning_rate = 0.001)
    ...
    def call(self, inputs, training=None):
        """generate fake_images by calling the generators defined in __init__

        Args:
            inputs tensor: _description_
            training (bool, optional): training mode or not. Defaults to None.

        Returns:
            tensor: fake images generated by generators
        """
        n = tf.shape(inputs)[0]
        z = tf.random.normal(shape=(n, self.n_latent_dims))
        fake_images = self.generator(z, training=training)
        return fake_images
    ...
    def train_step(self, images_real):
        ...
        Args:
            images_real (tensor): real images
